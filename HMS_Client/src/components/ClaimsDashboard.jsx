...
export default function ClaimsDashboard() {
  ...
  const [claimAmount, setClaimAmount] = useState(""); // input for amount
  ...
  // Create Claim using billId, policyId, and amount
  const createClaim = () => {
    if (!policyId || !billId || !claimAmount)
      return alert("Enter Policy ID, Bill ID, and Claim Amount");
      ...
      <div className="space-y-4">
        {claims.map((claim) => (
          <div key={claim.claimId} className="bg-white p-4 rounded shadow">
            ...
            <p>
              <strong>Bill ID:</strong> {claim.billId}
            </p>
            <p>
              <strong>Date:</strong> {new Date(claim.claimDate).toLocaleString()}
            </p>
          </div>
        ))}
      </div>
